...
from extensions_ref.test import weight
...
class Qwen2TransformerBlock:
    def __init__(
        ...
        self.self_attention = Qwen2MultiHeadAttention(
    ...
    def __call__(
        ...
        laynorm_output = self.input_layernorm(x)
        attention_output = self.self_attention(laynorm_output, offset=offset, mask=mask)
        x = x + attention_output
        post_attention_layernorm_output = self.post_attention_layernorm(x)
        mlp_output = self.mlp(post_attention_layernorm_output)
        final_output = x + mlp_output
        return final_output
